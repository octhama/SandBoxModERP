 ...
 * Copyright (C) 2024-05-22 Hasler Tehou <hasler.tehou@octhama.tech>
 *
...
/**
 *      \file       todolist/scripts/todolist.php
 *		\ingroup    todolist
 *      \brief      This file is a command line script for module Todolist. You can execute it with:
 *      			php todolist/scripts/todolist.php
 */
...
function getTodos(): array
{
    ...
    if (file_exists('todos.csv')) {
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $completed = isset($row[4]) && (bool)$row[4];
            $todos[] = [
                'id' => $id,
                'task' => $task,
                'date' => $date,
...
function addTodo(string $task, string $date, string $time): void
{
    $todos = getTodos();
    $newTodo = [
        'id' => getNewId(),
...
function editTodo(string $id, string $task, string $date, string $time): void
{
    $todos = getTodos();
    foreach ($todos as &$todo) {
        if ($todo['id'] === $id) {
